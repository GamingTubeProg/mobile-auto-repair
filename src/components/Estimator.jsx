 ...
 *     Called when the user clicks "Request a precise quote".
...
const Estimator = ({ onRequestPreciseQuote }) => {
  ...
  useEffect(() => {
    ...
    getMakes().then(list => {
      if (!cancelled) { setMakes(list); setMakesLoading(false); }
    });
    return () => { cancelled = true; };
  }, []);

  // Load models when year + make change
  useEffect(() => {
  ...
  // Reset model when year/make changes
  ...
  return (
    <div className="estimator solid-box">
      ...
      <div className="est-body">
        ...
        {step === 3 && (
          <div className="est-panel">
            <h4 className="est-panel-title">
              Your <span className="title-accent">
                {FEATURES.ESTIMATOR_SHOW_PRICE ? 'Instant Estimate.' : 'Request Summary.'}
              </span>
            </h4>
            <p className="est-panel-hint">
              {FEATURES.ESTIMATOR_SHOW_PRICE
            ...
            {FEATURES.ESTIMATOR_SHOW_PRICE && estimate && (
              <div className="est-quote">
                <span className="est-quote-label">Preliminary estimate (CAD) — labour + common parts</span>
        ...
        {/* BACK / NEXT NAVIGATION (steps 0–2) */}
        {step < 3 && (
